...
def add_plot_params_box(fig, args):
    """Add a summary of plot parametersin plotting area."""
    lines = []
...
def detect_softclip_counts(bam_file, region, min_clip=5):
    """Count clipped reads per position across a region and returns a df with position, softclip_count."""
    softclip_counts = {}
...
def calculate_coverage_stats(regions, bam_file):
    """Compute mean, median and sd from depth of coverage for each locus."""
    data_list = []
    try:
...
def calculate_nucleotide_counts(bam_file, chromosome, position):
    ...
    # count_coverageis  0-based start, and doesn't take the last value of interval
    a, c, g, t = samfile.count_coverage(chromosome, start=pos-1, end=pos, quality_threshold=13)
    samfile.close()
    counts = {'A': int(a[0]) if len(a)>0 else 0,
...
def get_totalspan(regions, locus_name):
    """Function to compute the total span of a target locus."""
    locus_intervals = []
...
def region_median(coverage, start, end):
    """Compute median coverage in [start, end) from a coverage dic."""
...
def expand_interval(bam_file, chrom, start, end,
    ...
    # Open BAM and fetch coverage
    samfile = pysam.AlignmentFile(bam_file, "rb")
    original_left, original_right = start, end
    target_len = original_right - original_left

    left_max_ext = original_left - max_extension if max_extension else max(0, original_left - target_len*10)
    right_max_ext = original_right + max_extension if max_extension else original_right + target_len*10
    fetch_start = max(0, left_max_ext)
    fetch_end = right_max_ext

    if verbose:
        print(f"[DEBUG] Fetching coverage for {chrom}:{fetch_start}-{fetch_end} ...")
    cov_tuple = samfile.count_coverage(chrom, start=fetch_start, end=fetch_end, quality_threshold=13)
    ...
    # Function to detect consecutive drops within a round
    def has_consecutive(probe_covs, threshold, min_run):
        run = 0
        for c in probe_covs:
            if c < threshold:
                run += 1
    ...
    while not (left_done and right_done):
        ...
        # Build probe intervals relative to coverage array
        left_starts = [max(0, int(left_boundary - (i + 1) * left_spacing - offset)) for i in range(probe_number)]
        left_ends = [s + probe_size for s in left_starts]
        right_starts = [max(0, int(right_boundary + i * right_spacing - offset)) for i in range(probe_number)]
        right_ends = [s + probe_size for s in right_starts]

        # Probe coverage means
        left_covs = np.array([coverage_array[s:e].mean() if e>s else 0 for s, e in zip(left_starts, left_ends)])
        right_covs = np.array([coverage_array[s:e].mean() if e>s else 0 for s, e in zip(right_starts, right_ends)])
        ...
        # Check stop per sideconsecutive in this round
        stop_left = has_consecutive(left_covs, target_cov, stop_drops)
        stop_right = has_consecutive(right_covs, target_cov, stop_drops)

        # Verbose output
        if verbose:
            print(f"[DEBUG] Round {rounds}: L={left_boundary}-{left_boundary}, R={right_boundary}-{right_boundary}")
            print(f"        left_drops={left_drops}, right_drops={right_drops}, "
        ...
        if not stop_left:
            if left_nulls >= null_threshold:
                ...
                rounds_no_drop_left = rounds_no_drop_left + 1 if left_drops == 0 else 0
        else:
            rounds_no_drop_left = 0
        ...
        if not stop_right:
            if right_nulls >= null_threshold:
                ...
                rounds_no_drop_right = rounds_no_drop_right + 1 if right_drops == 0 else 0
        else:
            rounds_no_drop_right = 0
        ...
        if rounds > 1e6:
            if verbose: print("[WARNING] Maximum rounds exceeded, stopping.")
            break
    ...
    if verbose:
        print(f"[RESULT] Final interval: {chrom}:{left_boundary}-{right_boundary} | Rounds={rounds}")
...
def main():
    ...
    required=True, 
    help="Four columns file (tab delimited, as bed format: 'chromosome\tstart\tstop\tlocus_name') containing regions of \
    ...
    required=True, 
    help="Name of the region to use for normalisation in the region file.")
    ...
    parser.add_argument("--plot-threshold", 
        type=float, 
        default= 1.4, 
        help="Treshold value to consider a duplication. Default value is 1.4, i.e. the expected normalised coverage of a heterozygous diploid \
            organism possessing a mono-copy and a duplicated copy of a given locus.")
    ...
    parser.add_argument("--probe-symmetric",
        action="store_true",
        help="Enable symmetric extension of the interval (default: OFF)")
                    ...
                    if args.plot_interval and args.plot:
                        ...
                        with pysam.AlignmentFile(bam_file, "rb") as samfile:
                            ...
                            str_incr_span = expand_interval(
                                ...
                                symmetric=args.probe_symmetric) #symmetry off by default 
                        ...
                        with pysam.AlignmentFile(bam_file, "rb") as samfile:
                            depth_data = [(p.pos, p.n) for p in ssamfile.pileup(region=str_incr_span, min_base_quality=13, min_mapping_quality=20)]
